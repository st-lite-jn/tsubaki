...
<div class="is-layout-flow wp-block-query">
	<ul class="is-layout-flow is-custom-block-query wp-block-post-template">
<?php while ( $the_query -> have_posts()) : $the_query -> the_post(); ?>
<?php
	//サムネイル画像を取得
	$thumbnail = get_post_thumbnail_id( get_the_ID())
			   ? wp_get_attachment_image( get_post_thumbnail_id( get_the_ID() ) , 'medium', false , array('class'=>'attachment-post-thumbnail size-post-thumbnail wp-post-image'))
			   : false;
	$parmalink = get_the_permalink( get_the_ID() );
	if($thumbnail) {
		$thumbnail = "<figure class=\"wp-block-post-featured-image\">
						<a href\"{$parmalink}\">{$thumbnail}</a>
					  </figure>";
	}
...
<li class="wp-block-post">
	<div class="wp-block-columns alignwide">
		<div class="is-layout-flow wp-block-column is-thumbnail" style="flex-basis:25%">
			<?php echo $thumbnail;?>
		</div>
		<div class="is-layout-flow wp-block-column" style="flex-basis:75%">
			<div class="is-layout-flow wp-block-group is-style-row-gap-small">
